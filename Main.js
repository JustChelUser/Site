...
app.get("/authorization", function (request, response) {
  response.sendFile(__dirname + "/authorization.html");
});
...
//загрузка данных с формы регистрации 
...
//загрузка данных с формы обратной связи 
...
//загрузка данных с формы авторизации
app.post("/authorization.html", urlencodedParser, function (request, response) {
  {
    if (!request.body) return response.sendStatus(400);
    const userLogin = request.body.userLogin;
    const userPassword = request.body.userPassword;
    ...
    pool.query(sql, [userLogin], function (err, results) {
      ...
      {   
        fs.readFile("authorization.html", "utf8", function (error, data) {
